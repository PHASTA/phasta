...
/*
typedef struct DCPrimContext_ *DCPrimContext;
struct DCPrimContext_ {
  CeedScalar lambda;
  CeedScalar mu; 
  CeedScalar k;   
  CeedScalar cv;   
...
CEED_QFUNCTION(IFunction_DCPrim)(void *ctx, CeedInt Q,
  ...
  const CeedInt stabilization = context[8];
/*
  DCPrimContext context = ctx;
...
*/
  ...
  for (CeedInt i=0; i<Q; i++) {
    ...
    CeedScalar TauE;
    CeedScalar stab[5][3];               
// I don't think this works    Advection2dContext context = ctx;
